    ...
    func testAggregateTargets() {
        describe {
            ...
            $0.it("generates aggregate targets") {
                ...
                try expect(aggregateTargets[0].dependenciesReferences.count) == 2

                try expect(aggregateTargets[1].name) == "AggregateTarget2"
                try expect(aggregateTargets[1].dependenciesReferences.count) == 1

                try expect(nativeTargets[2].name) == "Other"
                try expect(nativeTargets[2].dependenciesReferences.count) == 1
                
                try expect(nativeTargets[3].name) == "Other2"
                try expect(nativeTargets[3].dependenciesReferences.count) == 2
    ...
    func testTargets() {
        describe {
            ...
            $0.it("generates dependencies") {
                ...
                try expect(dependencies[0].targetReference) == nativeTargets.first { $0.name == app.name }!.reference
                try expect(dependencies[1].targetReference) == nativeTargets.first { $0.name == framework.name }!.reference
            }
